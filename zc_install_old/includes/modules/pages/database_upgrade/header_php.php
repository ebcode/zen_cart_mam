...
//          if (!$dbinfo->version138 || $dbinfo->version139) continue;  // if prerequisite not completed, or already done, skip
	...
	case '1.3.9':  // upgrading from v1.3.9 TO 1.3.9_Multiple_Addresses_Mod  //Multiple Addresses Mod
//          if (!$dbinfo->version138 || $dbinfo->version139) continue;  // if prerequisite not completed, or already done, skip
          $sniffer_file = '_upgrade_zencart_139_to_139_Multiple_Addresses_Mod.sql';
          if (ZC_UPG_DEBUG2==true) echo $sniffer_file.'<br>';
          $got_v1_3_9_Multiple_Addresses_Mod = true; //after processing this step, this will be the new version-level
          $db_upgraded_to_version='1.3.9_Multiple_Addresses_Mod';
          break;
       ...
       $nothing_to_process=true;
        ...
        if (!$zc_install->fatal_error && isset($_POST['adminid']) && isset($_POST['adminpwd'])) {
			...
			echo "expecting: ". DB_TYPE . $sniffer_file;
