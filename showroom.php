...
//require('includes/functions/extra_functions/zen_lightbox.php');




//require('includes/classes/bmz_image_handler.class.php');
...
//detect $_SESSION['byob'] and offer to empty basket or redirect to checkout
    ...
    $q = "select p.products_image, p.products_image_2, products_quantity,  p.products_price, p.sold, pd.* from products p left join products_description pd on p.products_id = pd.products_id where p.master_categories_id = '54' order by products_sort_order";
...
while ($row = mysql_fetch_array($rs)){
     ...
     //$x = display_basket($row, $zz);
        ...
        //get image sizes
//$image_file = DIR_WS_IMAGES . $row['products_image'];

//echo "<br> image = $image_file";
...
//echo "<br>tmp image = $tmp_image_file";
//$products_image_label = addslashes($row['products_name']);

$tmp_image_file = DIR_WS_IMAGES . $row['products_image_2'];

$products_name = $row['products_name'];

$rel = 'lightbox[]';  //[] for prev next buttons
...
$a = "<a href=\"" . zen_lightbox($tmp_image_file, addslashes($products_name), LARGE_IMAGE_WIDTH, LARGE_IMAGE_HEIGHT) . "\" rel=\"" . $rel . "\" title=\"" . ($products_name) . "\">";
   ...
   require($template->get_template_dir('tpl_byob.php',
        DIR_WS_TEMPLATE, 
        $current_page_base,'templates'). '/tpl_display_table.php');
