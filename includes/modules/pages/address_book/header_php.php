...
$addresses_query = "SELECT address_book_id, entry_title, entry_firstname as firstname, entry_lastname as lastname,
                           entry_company as company, entry_street_address as street_address,
                           entry_suburb as suburb, entry_city as city, entry_postcode as postcode,
                           entry_state as state, entry_zone_id as zone_id, entry_country_id as country_id, entry_phone as phone
                    FROM   " . TABLE_ADDRESS_BOOK . "
                    WHERE  customers_id = :customersID
...
//Multiple Addresses Mod - for going back to checkout page from address book 'back' button
$back_btn_checkout=0;
if(isset($_GET['ref']) || isset($_SESSION['back_btn_checkout'])){
	if($_GET['ref']=='checkout'){
		$back_btn_checkout=1;
		$_SESSION['back_btn_checkout']=1;
		//echo "<br>back to checkout<br>";
	} else {
		if($_GET['ref']=='account'){
			unset($_SESSION['back_btn_checkout']);
			$back_btn_checkout=0;
		} else {
			$back_btn_checkout=1;
		}
