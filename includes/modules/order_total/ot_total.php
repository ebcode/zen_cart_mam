 ...
 */
  class ot_total {
    ...
    function process() {
	  ...
	  //Multiple Addresses Mod  -- there's probably a better way to do this.  this is based on splitting up the order into multiple
	  //						   orders for each address, and collecting the "grand total" in the $_SESSION.
	  if(isset($_SESSION['multiple_addresses'])){
	   if(!$this->called){
	   		$this->called=1; //otherwise it gets called twice
			$_SESSION['multiple_addresses_grand_total']+=$order->info['total'];
	   }
	  }
