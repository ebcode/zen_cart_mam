...
class upsxml {
    ...
    function quote($method = '') {
        global $db, $HTTP_POST_VARS, $order, $shipping_weight, $shipping_num_boxes, $total_weight, $boxcount, $cart;
		...
		//echo "<br>UPS DEST:".$order->delivery['city'].",". $state.",". $order->delivery['country']['iso_code_2'].",". $order->delivery['postcode'];
    ...
    function _upsOrigin($city, $stateprov, $country, $postal){
		...
		//echo "origin: $city,$stateprov,$country,$postal";
    ...
    function _upsDest($city, $stateprov, $country, $postal) {
		...
		//echo "<br>dest: $city,$stateprov,$country,$postal";
    ...
    function getPackages() {
        global $db;
        $packages = array();
        $packages_query = $db->Execute("select * from " . TABLE_PACKAGING . " order by package_cost;");
        while ($package = mysql_fetch_array($packages_query)) {
            $packages[] = array(
