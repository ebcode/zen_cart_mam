...
// class constructor
    function item() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_ITEM_ZONE > 0) ) {
        ...
        if($order->info['subtotal'] >= 24.95){  //only enable free shipping over 24.95
            $this->enabled = true;
        } else {
            $this->enabled = false;
        }      

      //$this->enabled = true; //elibird 7/6/2014
    }
...
// class methods
    function quote($method = '') {
      ...
      $zipcode_table = constant('MODULE_SHIPPING_ZIPSHIP_CODES_' . 1);
        
      $zipcode_zones = split("[,]", $zipcode_table);
       
      //echo "here!<br>";
        
      //var_dump($zipcode_zones);
/*
        echo "<pre>";
        var_dump($order->delivery['address_book_id']);      
echo "</pre>";

echo "<pre>";
        var_dump($order->customer['default_address_book_id']);      
echo "</pre>";
*/

      $dest_zipcode = $order->delivery['postcode'];
      $num_zones = count($zipcode_zones);
      $dest_zone = -1;
      
     for ($i=0; $i<$num_zones; $i++) {
        if ($dest_zipcode == $zipcode_zones[$i]) {
          $dest_zone = $i;
          //echo "<br> dest_zone = $i, zipcode = " . $zipcode_zones[$i];
          break;
        }
      }      
      
if($dest_zone == -1 || ($order->delivery['address_book_id'] !== $order->customer['default_address_book_id'])){
    //echo "not available";
 $this->quotes['error'] = 'not available';
 return $this->quotes;
}
