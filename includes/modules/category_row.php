...
if ($num_categories > 0) {
  if ($num_categories < MAX_DISPLAY_CATEGORIES_PER_ROW || MAX_DISPLAY_CATEGORIES_PER_ROW == 0) {
    ...
    //--elibird 2/12/2014
    // changing display of images to dropdown
    $select = '<select name="holiday" style="font-size: 16px;" onchange="document.location=this.value"><option>select a holiday</option>';
  ...
  while (!$categories->EOF) {
    ...
    $select .= '<option value="'.zen_href_link(FILENAME_DEFAULT, $cPath_new).'">' . $categories->fields['categories_name'] . '</option>';
