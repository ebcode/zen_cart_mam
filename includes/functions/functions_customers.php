...
// TABLES: address_format
  function zen_address_format($address_format_id, $address, $html, $boln, $eoln, $legend=false) {
    global $db;
    ...
    $phone = zen_output_string_protected($address['phone']);
...
// Text Mode
	...
	//$address_out .= $cr.$phone;
	
	if($legend){
		$address_out = '<legend class="address_number">'.$address['title'].'</legend>'.$address_out;
	}
...
// TABLES: customers, address_book
  function zen_address_label($customers_id, $address_id = 1, $html = false, $boln = '', $eoln = "\n", $legend = false) {
    ...
    $address_query = "select entry_firstname as firstname, entry_lastname as lastname,
                             ...
                             entry_country_id as country_id, entry_title as title
                      from " . TABLE_ADDRESS_BOOK . "
                      where customers_id = '" . (int)$customers_id . "'
                      and address_book_id = '" . (int)$address_id . "'";  //Multiple Addresses Mod

    $address = $db->Execute($address_query);

    $format_id = zen_get_address_format_id($address->fields['country_id']);
    return zen_address_format($format_id, $address->fields, $html, $boln, $eoln, $legend);
  }
