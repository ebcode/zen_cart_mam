...
class shoppingCart extends base {
  ...
  var $contents;
  /**
   * shopping cart contents backup
   * @var decimal
   */
  var $contents_tmp;  //Multiple Addresses Mod
  /**
  ...
  function restore_contents() {
    ...
    $products_query = "select products_id, customers_basket_quantity, customers_basket_id
                         from " . TABLE_CUSTOMERS_BASKET . "
                         where customers_id = '" . (int)$_SESSION['customer_id'] . "' order by customers_basket_id";  //Multiple Addresses Mod
    ...
    while (!$products->EOF) {
	  ...
	  //Multiple Addresses Mod, restores $_SESSION[$multiple_addresses] array from database
	  $product_id = (int)$products->fields['products_id'];
	  
	  $q = "select * from ". TABLE_MULTIPLE_ADDRESSES_CUSTOMERS_BASKETS_PRODUCTS_ORDERS.
	  " where products_id='".$product_id."' and customers_basket_id = " .
	  $products->fields['customers_basket_id'] ." order by multiple_addresses_customers_baskets_products_orders_id";
	  
	  $rs = $db->Execute($q);
	  
	  $how_many_addresses = $rs->RecordCount();
	    
		if($how_many_addresses){  
			  
		   $multiple_addresses[$product_id]['how_many_addresses']=$how_many_addresses;
		  
		  if($how_many_addresses>1){
		  	 do{
		  	 	$multiple_addresses[$product_id]['addresses'][]=array('address_book_id'=>$rs->fields['address_book_id'],'qty'=>$rs->fields['products_quantity_for_this_address'],'shipping_method'=>$rs->fields['shipping_method'], 'shipping_module_code'=>$rs->fields['shipping_module_code']);
		  	    $rs->MoveNext();
			 } while(!$rs->EOF);
		  } else {
		  	$multiple_addresses[$product_id]['addresses']=array('address_book_id'=>$rs->fields['address_book_id'],'qty'=>$rs->fields['products_quantity_for_this_address'],'shipping_method'=>$rs->fields['shipping_method'], 'shipping_module_code'=>$rs->fields['shipping_module_code']);
   ...
   * and if the customer is logged in, also adds to the database stored
   * cart.
   *
  ...
  function add_cart($products_id, $qty = '1', $attributes = '', $notify = true, $byob_id=0) {  //--ebird
    ...
    if ($this->in_cart($products_id)) {
      ...
      if (isset($_SESSION['customer_id'])) {
        ...
        */
      	$sql = "insert into " . TABLE_CUSTOMERS_BASKET . "
                              (customers_id, products_id, customers_basket_quantity,
                              customers_basket_date_added, byob_id)
                              values ('" . (int)$_SESSION['customer_id'] . "', '" . zen_db_input($products_id) . "', '" .
      	                              $qty . "', '" . date('Ymd') . "', '$byob_id')";
  ...
  function remove($products_id) {
    ...
    if(in_array($products_id, $byob_product_ids)){
    	unset($_SESSION['byob']);
	...
	//Multiple Addresses Mod -- unset multiple addresses when basket is empty
	if(isset($_SESSION['multiple_addresses'])){
		unset($_SESSION['multiple_addresses'][(int)$products_id]);
		if(!count($_SESSION['multiple_addresses'])){
...
//die('I see Update Cart: ' . $_POST['products_id'][$i] . ' add qty: ' . $add_max . ' - cart qty: ' . $cart_qty . ' - newqty: ' . $new_qty);
		...
		//echo "<br><br>1635 add cart and byob = " . $_SESSION['byob_id'];  //elibird
	
          $this->add_cart($_POST['products_id'][$i], $new_qty, $attributes, false, $_SESSION['byob_id']);
...
//          $messageStack->add('header', 'REMOVE ME IN SHOPPING CART CLASS BEFORE RELEASE<br/><BR />' . ERROR_CORRECTIONS_HEADING . $the_list, 'error');
          ...
          if(isset($_POST['byob'])){
          	...
          	$q = "insert into bab_orders (basket_id) values ('$product_id')";
          		
          	mysql_query($q) or die(__LINE__.':'.mysql_errror());
          		
          	$byob_id = mysql_insert_id();
          	
          	$_SESSION['byob_id']=$byob_id;
          	
          	//echo __LINE__." byob_id = $byob_id<br>";
			...
			$app_q = '';
          	...
          	foreach($_POST as $k => $v){
          		...
          		if($x=='item_'){
	          		...
	          		if((int)$v>0){ //quantity > 0
	          			...
	          			$app_q  .= "('$byob_id', '$item_id', '$quantity'),";
  			...
  			if(!$app_q){
  				echo "Whoops!  You submitted an empty basket!  <a href=\"#\" onclick=\"window.history.go(-1);\">go back</a>";
				die();
  			}
			
  			
  			$app_q = substr($app_q, 0, strlen($app_q)-1); //trailing comma
          ...
          //echo "<br> calling add_cart with byob_id = $byob_id";
          
          //echo "<br><br>1792 add cart: byob_id = $byob_id";  //elibird
          
          $this->add_cart($_POST['products_id'], $this->get_quantity(zen_get_uprid($_POST['products_id'], $real_ids))+($new_qty), $real_ids, true, $byob_id);
...
//die('I see Buy Now Cart: ' . $add_max . ' - cart qty: ' . $cart_qty . ' - newqty: ' . $new_qty);
        ...
        if ((zen_get_products_quantity_order_max($_GET['products_id']) == 1 and $this->in_cart_mixed($_GET['products_id']) == 1)) {
          ...
          //                                    $_SESSION['cart']->add_cart($_GET['products_id'], $_SESSION['cart']->get_quantity($_GET['products_id'])+$add_qty);
           ...
           //echo "<br><br>1852 add cart: byob_id = $byob_id";  //elibird
...
//echo 'I SEE actionMultipleAddProduct: ' . $prodId . '<br>';
          ...
          if (($add_max == 1 and $cart_qty == 1)) {
            ...
            if (($new_qty + $cart_qty > $add_max) and $add_max != 0) {
             ...
             //echo "<br><br>1897 add cart: byob_id = $byob_id";  //elibird
    ...
    if ($_SESSION['customer_id'] && isset($_GET['pid'])) {
      if (zen_has_product_attributes($_GET['pid'])) {
        zen_redirect(zen_href_link(zen_get_info_page($_GET['pid']), 'products_id=' . $_GET['pid']));
		 ...
		 //echo "<br><br>1996 add cart: byob_id = $byob_id";  //elibird
...
//            $messageStack->add_session('shopping_cart', 'A: ' . ERROR_QUANTITY_ADJUSTED . zen_get_products_name($products) . ' - ' . $old_quantity . ' => ' . $new_qty, 'caution');
  ...
  //need some kind of function to mask the fact that this order is being sent to multiple addresses when calculating totals for this cart...
  // something that will temporarily set the $contents to contain only one product per address, with quantity, as well as setting the 
  // $_SESSION['sendto'] address, the shipping method, module code, and cost. 
  // this function will be called right before $order_total_modules->process() in modules/checkout_payment/header.php
  // will need another function to restore the contents
  
  function save_contents(){
  	$this->contents_tmp = $this->contents;
	$_SESSION['shipping_tmp']=$_SESSION['shipping'];
	$_SESSION['sendto_tmp']=$_SESSION['sendto'];
  }
  
  function revert_contents(){
  	$this->contents = $this->contents_tmp;
	$_SESSION['shipping']=$_SESSION['shipping_tmp'];
	$_SESSION['sendto']=$_SESSION['sendto_tmp'];
  }
  
  function set_temporary_contents($product_id, $qty, $sendto, $shipping_array){ //hmmm....
  		
  	$_SESSION['cart']->contents[$product_id]['qty']=$qty;
  	$_SESSION['shipping']=$shipping_array;
...
//Multiple Addresses Mod - !TODO  have to fix anything here regarding shipping, tax and weight