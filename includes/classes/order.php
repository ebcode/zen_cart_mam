...
class order extends base {
  ...
  function order($order_id = '') {
	...
	$this->multiple_addresses = array(); //Multiple Addresses Mod
  ...
  function query($order_id) {
    ...
    $order_query = "select customers_id, customers_name, customers_company,
                         ...
                         date_purchased, orders_status, last_modified, order_total, order_tax, ip_address, multiple_addresses
                        from " . TABLE_ORDERS . " 
                        where orders_id = '" . (int)$order_id . "'"; //Multiple Addresses Mod

    $order = $db->Execute($order_query);
	
	
	if($order->fields['multiple_addresses']){
		$totals_order_by = 'orders_total_id';
	} else {
		$totals_order_by = 'sort_order';
	}
	
    $totals_query = "select title, text, class, value
                         from " . TABLE_ORDERS_TOTAL . "
                         where orders_id = '" . (int)$order_id . "'
                         order by $totals_order_by";
    ...
    while (!$totals->EOF) {
      ...
      $this->totals[] = array('title' => ($totals->fields['class'] == 'ot_coupon' ? $zc_coupon_link . $totals->fields['title'] . '</a>' : $totals->fields['title']),
                              'text' => $totals->fields['text'], 'value' => $totals->fields['value'],
    ...
    $this->info = array('currency' => $order->fields['currency'],
                        ...
                        'multiple_addresses'=>$order->fields['multiple_addresses']
                        );
    ...
    if($this->info['multiple_addresses']){	//Multiple Addresses Mod				 
	      $index = 0;  
	      
		  /*
	      $q = "select ma.multiple_addresses_customers_baskets_products_orders_id, ma.*, pr.products_name 
	                                       ...
	                                       where ma.orders_id = '" . (int)$order_id . "'
	     ...
	     $q = "select ma.multiple_addresses_customers_baskets_products_orders_id, ma.customers_id, ma.orders_products_id,
	      		...
	      		ma.entry_zone_id as zone_id, pr.products_name 
	                                       ...
	                                       where ma.orders_id = '" . (int)$order_id . "'
	                                       order by multiple_addresses_customers_baskets_products_orders_id";
		  $multiple_addresses = $db->Execute($q);
	      
		  //echo "<br>line 188: sql = $q<br><br>";
		  
	      while(!$multiple_addresses->EOF){
	      	
	      	//var_dump($multiple_addresses);	
	      		//echo "<br>";
	      	
	      	//build multiple_addresses array for this order
			//echo "<br>index=$index";
			$this->multiple_addresses[$index]=$multiple_addresses->fields;
			//$this->multiple_addresses[$index]=  array('id' => $multiple_addresses->fields['products_id'],
									//	'orders_products_id' => $multiple_addresses->fields['orders_products_id'],
			//echo "<br>entry_company = ".$multiple_addresses->fields['entry_company'];
				
			$index++;
			$multiple_addresses->MoveNext();
			
	      }
		  
	      //var_dump($this->multiple_addresses);
  ...
  function cart() {
	//echo "<br>242, order class, cart function";
    global $db, $currencies;

    $this->content_type = $_SESSION['cart']->get_content_type();

    $customer_address_query = "select c.customers_firstname, c.customers_lastname, c.customers_telephone,
                                    c.customers_email_address, c.customers_default_address_id,  ab.entry_company, ab.entry_street_address,
    ...
    $shipping_address_query = "select ab.address_book_id, ab.entry_firstname, ab.entry_lastname, ab.entry_company,
...
*/
    $this->customer = array('default_address_book_id' => $customer_address->fields['customers_default_address_id'],
    ...
    $this->delivery = array('address_book_id' => $shipping_address->fields['address_book_id'],
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
       ...
       *********************************************/
      //echo "<br>quantity = ".$this->products[$index]['qty'];
      //echo "<br>final_price = ".$this->products[$index]['final_price'];
...
//        $tax_add = zen_round(($products_tax / 100) * $shown_price, $currencies->currencies[$this->info['currency']]['decimal_places']);
	...
	//die($_GET['main_page']);
	//$_SESSION['multiple_addresses_grand_total'] will always have the correct amount if it is set
	//echo "<br><br>order get = ". $_GET['main_page'] ."<br><br>";

	if(isset($_SESSION['multiple_addresses_grand_total']) && isset($_SESSION['multiple_addresses']) && ($_GET['main_page']!='checkout_process' && $_GET['main_page']!='checkout_confirmation' && $_GET['main_page']!='checkout_payment')){
		$this->info['total'] =$_SESSION['multiple_addresses_grand_total'];
	}
...
*/
  ...
  function create($zf_ot_modules, $zf_mode = 2) {
	...
	//Multiple Addresses Mod
	if(isset($_SESSION['multiple_addresses_grand_total']) && isset($_SESSION['multiple_addresses'])){
		$order_total = $_SESSION['multiple_addresses_grand_total'];
	} else {
    ...
    $sql_data_array = array('customers_id' => $_SESSION['customer_id'],
                            ...
                            'ip_address' => $_SESSION['customers_ip_address'] . ' - ' . $_SERVER['REMOTE_ADDR'],
                            'multiple_addresses' => isset($_SESSION['multiple_addresses'])?1:0  
                            ); //Multiple Addresses Mod
...
/*
	if(is_multi($zf_ot_modules)){  //Multiple Addresses Mod
		

		var_dump($_SESSION);
		echo "<br><br>";
*/
		//var_dump($zf_ot_modules);
		//die('is_multi');
		
	if(isset($_SESSION['multiple_addresses'])){	

		foreach($zf_ot_modules as $zf_ot_module){
			for ($i=0, $n=sizeof($zf_ot_module); $i<$n; $i++) {
		      $sql_data_array = array('orders_id' => $insert_id,
		                              'title' => $zf_ot_module[$i]['title'],
		                              'text' => $zf_ot_module[$i]['text'],
		                             // 'value' => (is_numeric($zf_ot_module[$i]['value'])) ? $zf_ot_module[$i]['value'] : '0',
		                             'value' => $zf_ot_module[$i]['value'],
		                              'class' => $zf_ot_module[$i]['code'],
		                              'sort_order' => $zf_ot_module[$i]['sort_order']);
				
			//	echo "<br>order, sql_data =";
  ...
  function  create_add_products($zf_insert_id, $zf_mode = false) {
    ...
    for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
	  ...
	  global $byob_product_ids;
	  
      if(in_array(zen_get_prid($this->products[$i]['id']),$byob_product_ids)){
      	$byob_id = $_SESSION['byob_id'];
		
		$this->products[$i]['byob_items']='';
		...
		while (!$byob_items->EOF) {
			...
			*/
			...
			$this->products[$i]['byob_items'].="\n".$byob_items->fields['nm']." ... x ". $byob_items->fields['quantity'] ."\n";
      ...
      $order_products_id = $db->Insert_ID();
		...
		//Multiple Addresses Mod
		if(isset($_SESSION['multiple_addresses'])){
			$q = "update ". TABLE_MULTIPLE_ADDRESSES_CUSTOMERS_BASKETS_PRODUCTS_ORDERS." set orders_products_id = ". $order_products_id.
				" where orders_id = ".$zf_insert_id;
			
			$db->Execute($q);
			
			$q = "select ma.address_book_id as book_id, ab.* from ".TABLE_MULTIPLE_ADDRESSES_CUSTOMERS_BASKETS_PRODUCTS_ORDERS.
				  " ma left join " . TABLE_ADDRESS_BOOK . " ab on ma.address_book_id = ab.address_book_id";
			
			$rs = $db->Execute($q);
			
			while(!$rs->EOF){  //storing addresses in this table in case the user deletes their address entry
				//entry_company = '".$rs->fields['entry_company']."',
				$q = "update ".TABLE_MULTIPLE_ADDRESSES_CUSTOMERS_BASKETS_PRODUCTS_ORDERS. 
				" set  entry_firstname = '".$rs->fields['entry_firstname']."', ".
				"entry_lastname = '".$rs->fields['entry_lastname']."', entry_phone = '".$rs->fields['entry_phone']."', entry_street_address = '".$rs->fields['entry_street_address']."', ".
				"entry_suburb = '".$rs->fields['entry_suburb']."', entry_postcode = '".$rs->fields['entry_postcode']."',".
				"entry_city = '".$rs->fields['entry_city']."', entry_state = '".$rs->fields['entry_state']."', ".
				"entry_country_id = '".$rs->fields['entry_country_id']."', entry_zone_id = '".$rs->fields['entry_zone_id'].
				"' where address_book_id = '".$rs->fields['address_book_id']."'";
				
				//echo "<br>create_add_products, line 882: q=$q";
		...
		//byob
		if(zen_get_prid($this->products[$i]['id'])==175){
...
/* END: ADD MY CUSTOM DETAILS */
	  ...
	  //byob --elibird
	  $this->products_ordered .= $this->products[$i]['byob_items'];
      ...
      '</td></tr>' . "\n";
	   ...
	   $this->products_ordered_html .= 
		'<tr>' . "\n" .
		 '<td></td><td>' . nl2br($this->products[$i]['byob_items']) . '</td></tr>';
  ...
  function send_order_email($zf_insert_id, $zf_mode) {
    ...
    //Multiple Addresses Mod, -- loop through multidimensional order_totals
    if(isset($_SESSION['multiple_addresses'])){	
		
    	foreach($order_totals as $order_total){
	    	for ($i=0, $n=sizeof($order_total); $i<$n; $i++) {
		      $email_order .= strip_tags($order_total[$i]['title']) . ' ' . strip_tags($order_total[$i]['text']) . "\n";
		      $html_ot .= '<td class="order-totals-text" align="right" width="100%">' . $order_total[$i]['title'] . '</td> ' . "\n" . '<td class="order-totals-num" align="right" nowrap="nowrap">' .($order_total[$i]['text']) .'</td> </tr>' . "\n" . '<tr>';
		    }
		}
		
		$email_order .= 'Grand Total:'. ' $' . (round($_SESSION['multiple_addresses_grand_total'],2)) . "\n";		     
		$html_ot .= '<td class="order-totals-text" align="right" width="100%">Grand Total:</td> ' . "\n" . '<td class="order-totals-num" align="right" nowrap="nowrap">' .(round($_SESSION['multiple_addresses_grand_total'],2)) .'</td> </tr>' . "\n" . '<tr>';
	...
	//Multiple Addresses Mod, TODO --need loop here
			 ...
			 if ($how_many>1){
			 	for($i=0;$i<$how_many;$i++){
					...
					$email_order .= EMAIL_SEPARATOR . "\n" .
				      zen_address_label($_SESSION['customer_id'], $address_book_id, 0, '', "\n") . "\n";
				}
			 } else {
...
/*
    //write out emails to email_queue folder  --elibird 3/7/2014
    $fh = fopen(DIR_FS_CATALOG . 'email_queue/emails.txt', 'a');

    $email_queue = array($this->customer['firstname'] . ' ' . $this->customer['lastname'], $this->customer['email_address'], EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id, $email_order, STORE_NAME, EMAIL_FROM, $html_msg, 'checkout', $this->attachArray);
    
    $email_queue = serialize($email_queue);
    ...
    */
    ...
    if (SEND_EXTRA_ORDER_EMAILS_TO != '') {
      ...
      zen_mail('', SEND_EXTRA_ORDER_EMAILS_TO, SEND_EXTRA_NEW_ORDERS_EMAILS_TO_SUBJECT . ' ' . EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id, $email_order . $extra_info['TEXT'], STORE_NAME, EMAIL_FROM, $html_msg, 'checkout_extra', $this->attachArray);
       /*     
       $email_queue = array('', SEND_EXTRA_ORDER_EMAILS_TO, SEND_EXTRA_NEW_ORDERS_EMAILS_TO_SUBJECT . ' ' .EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id, $email_order . $extra_info['TEXT'], STORE_NAME, EMAIL_FROM, $html_msg, 'checkout_extra', $this->attachArray);
