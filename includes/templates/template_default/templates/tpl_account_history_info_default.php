...
<?php
  ...
  if($order->info['multiple_addresses']){
	...
	?>   
	<div class="amount larger forward" style="border-top:4px double;"><?php echo $currencies->format($grand_total); ?></div>
	<div class="lineTitle larger forward" style="border-top:4px double;"><?php echo GRAND_TOTAL; ?></div>
  ...
  if ($order->delivery != false) {
	...
	//							to which addresses by which shipping methods
                ...
                if($order->info['multiple_addresses']){
                	...
                	//echo "multiple";
                	?>
                	<!-- <td class="main" style=""> </td> --><h3><?php echo DELIVERY_INFORMATION; ?></h3>
                	...
                	foreach($order->multiple_addresses as $a){
		    			...
		    			echo zen_address_format($format_id, $a, true, '', "\n", 0);
							//echo '<address class="address">'.$multiple_address_detail['entry_title'].'</address>';
						//echo '</address>';
...
<?php
    if (zen_not_null($order->info['shipping_method']) && !$order->info['multiple_addresses']) {
?>
