...
?>
  <tr>
    ...
    <td>
    	<?php if($orders['multiple_addresses']){
    		echo '<div>'.MY_ACCOUNT_MULTIPLE_ADDRESSES.'</div>';
			//var_dump($orders['multiple_address_details'] );
			
			//echo $orders['multiple_address_details'][0]['entry_title'];
			foreach($orders['multiple_address_details'] as $multiple_address_detail){
				
				$format_id = zen_get_address_format_id($multiple_address_detail['country_id']);
				echo '<address class="address">';
    			echo zen_address_format($format_id, $multiple_address_detail, true, '', "\n", 0);
			...
			foreach($orders['multiple_address_details'] as $multiple_address_detail){
				?>
				<address>
					<?php echo $multiple_address_detail['entry_title']; ?>
				</address>
				<?
			}
			*/
    	...
    	<address><?php echo zen_output_string_protected($orders['order_name']) . '<br />' . $orders['order_country']; ?></address>
    	
    	<?php
    	}
		
		/*
		 * Multiple Addresses Mod !TODO: echo multiple addresses here, get from orders table
    	multiple_addresses = <?php echo $orders['multiple_addresses']; ?>
		 */
...
<li><?php echo ' <a href="' . zen_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL') . '&ref=account">' . MY_ACCOUNT_ADDRESS_BOOK . '</a>'; //Multiple Addresses Mod ?></li>
