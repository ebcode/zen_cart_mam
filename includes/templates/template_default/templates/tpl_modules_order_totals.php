...
<?php 
	 ...
	 *  
  */
 $id = str_replace('_', '', $GLOBALS[$class]->code);
 
 $o = <<<EOF
 <div id="{$id}">
    <span class="totalBox larger forward">{$GLOBALS[$class]->output[$i]['text']}</span>
    <span class="lineTitle larger forward">{$GLOBALS[$class]->output[$i]['title']}</span>
</div>
<br class="clearBoth" />
EOF;
