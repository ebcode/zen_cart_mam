...
//          $update_downloads_query = "update " . TABLE_ORDERS_PRODUCTS_DOWNLOAD . " set download_maxdays='0', download_count='0' where orders_id='" . $_GET['oID'] . "' and orders_products_download_id='" . $_GET['download_reset_off'] . "'";
		...
		//get gift messages
		$q = "select * from gift_msg where orders_id = ".$_GET['oID'] . " order by gift_msg_id";
		$msg_rs = mysql_query($q) or die(mysql_error);
		
		//get phone numbers
		$q = "select * from gift_phn where orders_id = ".$_GET['oID'] . " order by gift_phn_id";
		$phn_rs = mysql_query($q) or die(mysql_error);
...
?>
	...
	//Multiple Addresses Mod -- if multiple addresses, display which products going 
	//							to which addresses by which shipping methods
                
                if($order->info['multiple_addresses']){
               
                	//echo "multiple";
                	?>
                	<td class="main" style=""><strong><?php echo ENTRY_SHIPPING_ADDRESS; ?></strong><br>
                	<table style="border:0;" cellpadding="2" cellspacing="0" >
                		<tr class="dataTableHeadingRow">
                		<th><?php echo TABLE_HEADING_PRODUCTS; ?></th>
                		<th><?php echo TABLE_HEADING_ADDRESS; ?></th>
                		<th><?php echo TABLE_HEADING_SHIPPING_METHOD; ?></th>
                		<th width="200px">Gift Messages</th>
                		</tr>
                	<?php             	
                	
					//var_dump($order->multiple_addresses);
                	
                	foreach($order->multiple_addresses as $a){
                		//var_dump($a);
                		?>
                		<tr><td><?php echo $a['products_quantity_for_this_address'] . ' x '. $a['products_name']; ?></td>
                		
                			
                			<td><address class="address" style="margin:2px; float:left; border:1px solid; padding:2px;">
                		<?php 
                		
                		$format_id = zen_get_address_format_id($a['country_id']);
    					echo zen_address_format($format_id, $a, true, '', "\n", 0);
                		
           				//var_dump($a);
						
						
                		//echo zen_address_label($a['customers_id'], $a['address_book_id'], 1, '', '<br />'); 
                		
                		?>
                	</address>
                	</td>
                	<td>
                	<?php echo $a['shipping_method'] . ', ' . $a['shipping_method_title']; ?>
                	</td>
						...
						<?php
						 ...
						 for($l=0;$l<$a['products_quantity_for_this_address'];$l++){
	                				$msg_row = mysql_fetch_array($msg_rs);
							$phn_row = mysql_fetch_array($phn_rs); 
							echo '<div style="border:1px solid; margin:2px;padding:2px;">'.$msg_row['msg'].'</div>';
							echo '<div style="border:1px solid; margin:2px;padding:2px;">'.$phn_row['phn'].'</div>';
						 }
						?>	
							<?php //echo $msg_row['msg']; ?>
					...
					echo "<br><br>Gift Message(s): <br><br>";
					
					for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
						
						
						for($l=0;$l<$order->products[$i]['qty'];$l++){
						
							$msg_row = mysql_fetch_array($msg_rs);
							$phn_row = mysql_fetch_array($phn_rs); 

							echo '<div style="border:1px solid; margin:2px; padding:2px;">'.$msg_row['msg'].'</div>';
							echo '<div style="border:0px solid; margin:2px;padding:2px;">phone: '.$phn_row['phn'].'</div>';
						}
...
<?php
    for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
      ...
      //byob --ebird
      if($order->products[$i]['byob']){
		foreach($order->products[$i]['byob'] as $byob_item){
			echo "<tr><td></td><td>". $byob_item['item_name']." x ".$byob_item['quantity']."</td></tr>";
		}
	  }
...
<?php
    ...
    if($order->info['multiple_addresses']){
        ...
        //echo "<br> sizeof = " . sizeof($order->totals);

		for ($i = 0, $n = sizeof($order->totals); $i < $n; $i++) {
	      echo '              <tr>' . "\n" .
	           '                <td align="right" class="'. str_replace('_', '-', $order->totals[$i]['class']) . '-Text">' . $order->totals[$i]['title'] . '</td>' . "\n" .
	           '                <td align="right" class="">' . $order->totals[$i]['text'] .'</td>' . "\n" .
	           '              </tr>' . "\n";
            //echo "<br> class = " . $order->totals[$i]['class'];
		  if($order->totals[$i]['class']=='ot_total'){
		  	$grand_total+=$order->totals[$i]['value'];
		  }
	    }
		
		echo '<tr><td style="border-top:4px double; font-weight:bold;">'.GRAND_TOTAL.'</td><td style="border-top:4px double; font-weight:bold;">'.$currencies->format(round($grand_total,2)).'</td></tr>';
    ...
    if (isset($_GET['cID'])) {
     ...
     /*
	  $orders_query_raw =   "select o.orders_id, o.customers_id, o.customers_name, o.payment_method, o.shipping_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, s.orders_status_name, ot.text as order_total" .
                            ...
                            where o.customers_id = '" . (int)$cID . "' and o.orders_status = s.orders_status_id and s.language_id = '" . (int)$_SESSION['languages_id'] . "' GROUP BY o.orders_id order by orders_id DESC";
	*/
	$orders_query_raw =   "select o.orders_id, o.customers_id, o.customers_name, o.payment_method, o.shipping_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, s.orders_status_name, o.order_total, o.multiple_addresses" .
                            $new_fields . "
                            from (" . TABLE_ORDERS_STATUS . " s, " .
                            TABLE_ORDERS . " o " .
                            $new_table . ")
...
//echo '<BR><BR>I SEE A: ' . $orders_query_raw . '<BR><BR>';
      ...
      $orders_query_raw = "select o.orders_id, o.customers_id, o.customers_name, o.payment_method, o.shipping_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, s.orders_status_name, ot.text as order_total" .
                          ...
                          $search . " GROUP BY o.orders_id order by o.orders_id DESC";
*/
	$orders_query_raw = "select o.orders_id, o.customers_id, o.customers_name, o.payment_method, o.shipping_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, s.orders_status_name, o.order_total, o.multiple_addresses" .
                          $new_fields . "
                          from (" . TABLE_ORDERS_STATUS . " s, " .
...
//echo '<BR><BR>I SEE B: ' . $orders_query_raw . '<BR><BR>';
      ...
      $orders_query_raw = "select " . $search_distinct . " o.orders_id, o.customers_id, o.customers_name, o.payment_method, o.shipping_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, s.orders_status_name, ot.text as order_total" .
                          ...
                          $search . " GROUP BY o.orders_id order by o.orders_id DESC";
*/
	$orders_query_raw = "select " . $search_distinct . " o.orders_id, o.customers_id, o.customers_name, o.payment_method, o.shipping_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, s.orders_status_name, o.order_total, o.multiple_addresses" .
                          $new_fields . "
                          from (" . TABLE_ORDERS_STATUS . " s, " .
                          TABLE_ORDERS . " o " .
                          $new_table . ")
...
//echo '<BR><BR>I SEE C: ' . $orders_query_raw . '<BR><BR>';
	...
	//$orders_query_raw .=" GROUP BY o.orders_id"; //Multiple Addresses Mod
	//echo '<BR><BR>I SEE C: ' . $orders_query_raw . '<BR><BR>';
...
//    $orders_query_numrows = '';
    ...
    if ((!isset($_GET['oID']) || (isset($_GET['oID']) && ($_GET['oID'] == $orders->fields['orders_id']))) && !isset($oInfo)) {
	  ...
	  if($orders->fields['multiple_addresses']){ //Multiple Addresses Mod
	  	$show_payment_type = $orders->fields['payment_module_code'] . '<br />' . MULTIPLE_ADDRESSES;
		$show_difference = zen_image(DIR_WS_IMAGES . 'icon_status_red.gif', TEXT_BILLING_SHIPPING_MISMATCH, 10, 10) . '&nbsp;';
...
//      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO . '<br /><br /><strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>');
    ...
    default:
      if (isset($oInfo) && is_object($oInfo)) {
        ...
        //Multiple Addresses Mod -- change shipping based on multiple_addresses field in order table
        
        if($oInfo->multiple_addresses){
			$contents[] = array('text' => '<br />' . ENTRY_SHIPPING . ' '  . MULTIPLE_ADDRESSES);
        } else {
