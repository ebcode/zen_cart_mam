...
<?php
  if ($action == 'new_product' or $action == 'new_product_meta_tags') {
	...
	if(isset($_GET['del_img'])){
		$img_to_del = (int)$_GET['del_img'];
		//echo "delete image!";
	
		//get image file
		if($img_to_del == 1){
			$img_name = "products_image";
			$img_label = "products_image_label";
		} else {
			$img_name = "products_image_".$img_to_del;
			$img_label = "products_image_".$img_to_del."_label";
		}
		
		$q = "select $img_name from products where products_id = '". (int)$_GET['pID']."'";
		
		$rs=mysql_query($q) or die(__LINE__. ' '. mysql_error());
		
		$row = mysql_fetch_array($rs) or die(__LINE__. ' '. mysql_error());
		
		$img_file = '../images/'.$row[$img_name];
		//echo "img_file = $img_file";
		if(file_exists($img_file)){
			//echo '<br>deleting '.$img_file;
			@unlink($img_file);
			
		}
		$q = "update products set $img_name ='', $img_label=''where products_id = '". (int)$_GET['pID']."' and $img_name = '".$row[$img_name]."'";
		//echo "<br>q=$q";
		//unlink('../images/'.)
