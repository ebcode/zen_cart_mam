...
if (!defined('IS_ADMIN_FLAG')) {
  die('Illegal Access');
    ...
    $parameters = array('products_name' => '',
                       ...
                       'products_model' => '',
                       'products_image' => '',
                       'products_image_label' => '',
                       'products_image_2' => '',
                       'products_image_2_label' => '',
                       'products_image_3' => '',
                       'products_image_3_label' => '',
                       'products_image_4' => '',
                       'products_image_4_label' => '',
                       'products_image_5' => '',
                       'products_image_5_label' => '',
                       'products_image_6' => '',
                       'products_image_6_label' => '',
                       'products_image_7' => '',
                       'products_image_7_label' => '',
                       'products_image_8' => '',
                       'products_image_8_label' => '',
                       'products_image_9' => '',
                       'products_image_9_label' => '',
                       'products_price' => '',
    ...
    if (isset($_GET['pID']) && empty($_POST)) {
      $product = $db->Execute("select pd.products_name, pd.products_description, pd.products_url,
                                      ...
                                      p.products_image, p.products_image_label, p.products_price, p.products_virtual, p.products_weight,
                                      ...
                                      p.products_image_2, p.products_image_2_label,
                                      p.products_image_3, p.products_image_3_label,
                                      p.products_image_4, p.products_image_4_label,
                                      p.products_image_5, p.products_image_5_label,
                                      p.products_image_6, p.products_image_6_label,
                                      p.products_image_7, p.products_image_7_label,
                                      p.products_image_8, p.products_image_8_label,
                                      p.products_image_9, p.products_image_9_label
              ...
              <tr>
                ...
                <td class="main">
                	...
                	if($pInfo->products_image !=''){
                		?>
                	<div style="float:left;margin:2px;padding:2px;border:1px solid;">
                		<?php
                	
                		echo $pInfo->products_image.'<br><img width="100" src="../images/'.$pInfo->products_image.'" /><br>label: '.$pInfo->products_image_label;
                	
                	?><br>
                	<?php
                	echo "<a onclick=\"return(confirm('ok to delete?'));\" href=\"./product.php?cPath=$cPath&product_type=".$_GET['product_type']."&pID=".$_GET['pID']."&action=new_product&page=".$_GET['page']."&del_img=1\">delete</a>";
//echo zen_draw_form('new_product', $type_admin_handler , 'cPath=' . $cPath . (isset($_GET['product_type']) ? '&product_type=' . $_GET['product_type'] : '') . (isset($_GET['pID']) ? '&pID=' . $_GET['pID'] : '') . '&action=new_product_preview' . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '') . ( (isset($_GET['search']) && !empty($_GET['search'])) ? '&search=' . $_GET['search'] : '') . ( (isset($_POST['search']) && !empty($_POST['search']) && empty($_GET['search'])) ? '&search=' . $_POST['search'] : ''), 'post', 'enctype="multipart/form-data"');
                	...
                	<?php 
                	/* --comment on php manual from Sam-Mauris Yong
                	function varvar($str){
    if(strpos($str,'->') !== false){
...
var_dump($$test2); // NULL
					 ...
					 */
					for($zz=2;$zz<7;$zz++){
						$img_var = 'pInfo->products_image_'.$zz;
						$img_label_var = 'pInfo->products_image_'.$zz.'_label';
						$products_img = varvar($img_var);
						$products_img_label = varvar($img_label_var);
						//var_dump($obj_var);
	                	//var_dump($pInfo['products_image_2']);
	                	if($products_img !=''){
	                		?>
	                	<div style="float:left;margin:2px;padding:2px;border:1px solid;">
	                	<?php
	                	
	                		echo $products_img.'<br><img width="100" src="../images/'.$products_img.'" /><br>label: '.$products_img_label;
	                	
	                	?><br><?php echo "<a onclick=\"return(confirm('ok to delete?'));\" href=\"./product.php?cPath=$cPath&product_type=".$_GET['product_type']."&pID=".$_GET['pID']."&action=new_product&page=".$_GET['page']."&del_img=$zz\">delete</a>"; ?>
	                	</div>
	                	<?php
						}
                	...
                	Image 1: <?php echo  zen_draw_file_field('products_image') . '&nbsp;' . ($pInfo->products_image !='' ? TEXT_IMAGE_CURRENT . $pInfo->products_image : TEXT_IMAGE_CURRENT . '&nbsp;' . NONE) . zen_draw_hidden_field('products_previous_image', $pInfo->products_image); ?>
                	<br>
                	Image 1 Label: <input name="products_image_label" value="<?php echo $pInfo->products_image_label; ?>">
                <?php
                	
                	for($zz=2;$zz<7;$zz++){
                		$prod_img = varvar('pInfo->products_image_'.$zz);
						$prod_img_label = varvar('pInfo->products_image_'.$zz.'_label');
						
                		?>
                			<br>
                	Image <?php echo $zz;?>: <?php echo zen_draw_file_field('products_image_'.$zz) . '&nbsp;' . ($prod_img !='' ? TEXT_IMAGE_CURRENT . $prod_img : TEXT_IMAGE_CURRENT . '&nbsp;' . NONE) . zen_draw_hidden_field('products_previous_image_'.$zz, $prod_img); ?>
                	<br>
                	Image <?php echo $zz;?> Label: <input name="products_image_<?php echo $zz;?>_label" value="<?php echo $prod_img_label; ?>">
                ...
                <!--
                	<br>
                	Image 2: <?php echo zen_draw_file_field('products_image_2') . '&nbsp;' . ($pInfo->products_image_2 !='' ? TEXT_IMAGE_CURRENT . $pInfo->products_image_2 : TEXT_IMAGE_CURRENT . '&nbsp;' . NONE) . zen_draw_hidden_field('products_previous_image_2', $pInfo->products_image_2); ?>
                	<br>
                	Image 2 Label: <input name="products_image_2_label" value="<?php echo $pInfo->products_image_2_label; ?>">
               -->
