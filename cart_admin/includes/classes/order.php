  ...
  class order {
    ...
    function query($order_id) {
      ...
      $order = $db->Execute("select cc_cvv, customers_name, customers_company, customers_street_address,
                                    ...
                                    order_total, order_tax, ip_address, multiple_addresses
                             from " . TABLE_ORDERS . "
                             where orders_id = '" . (int)$order_id . "'");  //Multiple Addresses Mod

		//, order_products_multiple_addresses_id
		//var_dump($order);
		
		if($order->fields['multiple_addresses']){
			$order_totals_by='orders_total_id';
		} else {
			$order_totals_by='sort_order';
		}
	   
      $totals = $db->Execute("select title, text, class, value
                              from " . TABLE_ORDERS_TOTAL . "
                              where orders_id = '" . (int)$order_id . "'
                              order by $order_totals_by");
	  ...
	  if($this->info['multiple_addresses']){					 
	      $index = 0;  //Multiple Addresses Mod
	      ...
	      $q = "select ma.multiple_addresses_customers_baskets_products_orders_id, ma.customers_id, ma.orders_products_id,
	      		...
	      		ma.entry_zone_id as zone_id, ma.entry_phone as phone, pr.products_name 
...
// convert quantity to proper decimals - account history
		...
		*/
          $this->products[$index] = array('qty' => $new_qty,
          		...
          		'final_price' => $orders_products->fields['final_price'],
          		'product_is_free' => $orders_products->fields['product_is_free'],
          		'byob_id' => $orders_products->fields['byob_id']);
        ...
        $this->products[$index]['byob']=array();
        
        //echo "byob_id = ".$orders_products->fields['byob_id'];
        
        if($orders_products->fields['byob_id']){
        	$subindex = 0;
        	$byob_items = $db->Execute("select bo.quantity, bo.item_id, bi.* from bab_order_items bo left join bab_items bi on bo.item_id = bi.id where bo.bab_order_id = '".$orders_products->fields['byob_id']."'");
        	
        	//die("q = "."select * from bab_order_items where bab_order_id = '".$orders_products->fields['byob_id']."'");
        	...
        	//var_dump($this->products[$index]['byob']);
