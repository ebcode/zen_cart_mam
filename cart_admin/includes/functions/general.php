...
//    }
  ...
  function zen_remove_order($order_id, $restock = false) {
	...
	$db->Execute("delete from " . TABLE_MULTIPLE_ADDRESSES_CUSTOMERS_BASKETS_PRODUCTS_ORDERS . "
...
function zen_get_products_image_2($product_id) {
    global $db;
    $product_image = $db->Execute("select products_image_2
                                   from " . TABLE_PRODUCTS . "
                                   where products_id = '" . (int)$product_id . "'");

    return $product_image->fields['products_image_2'];
  }
