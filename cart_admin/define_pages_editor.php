...
$homepage_config = $db->Execute('select file from home_page_config', 1);

//var_dump($homepage_config->fields['file']);

$selected_homepage = $homepage_config->fields['file'];

//echo "<br> GET vars:";
//var_dump($_GET);

if(isset($_GET['home_page_file'])){
	
	$selected_homepage = mysql_real_escape_string($_GET['home_page_file']);
	
	$db->Execute("update home_page_config set file = '$selected_homepage'");
...
//echo 'I SEE ' . $check_directory[$i] . '<br>';
  ...
  if ($action == 'new_page') {
    ...
    /*
    echo "<br> looking in : ". DIR_FS_CATALOG . DIR_WS_LANGUAGES . $_SESSION['language'] . '/html_includes/';
*/
      ...
      <tr>
      	<td>
      		<form action="" method="get">
      			<input type="radio" name="home_page_file" id="home_page_file_1" value="define_main_page.php" <?php if($selected_homepage=='define_main_page.php'){ ?> checked="checked" <?php } ?> /><label for="home_page_file_1">define_main_page.php</label> &nbsp; &nbsp;
      			<input type="radio" name="home_page_file" id="home_page_file_2" value="define_main_page_2.php" <?php if($selected_homepage=='define_main_page_2.php'){ ?> checked="checked" <?php } ?> /><label for="home_page_file_2">define_main_page_2.php</label> &nbsp; &nbsp; 
      			<input type="radio" name="home_page_file" id="home_page_file_3" value="define_main_page_3.php" <?php if($selected_homepage=='define_main_page_3.php'){ ?> checked="checked" <?php } ?> /><label for="home_page_file_3">define_main_page_3.php</label> &nbsp; &nbsp;
      			<input type="radio" name="home_page_file" id="home_page_file_4" value="define_main_page_4.php" <?php if($selected_homepage=='define_main_page_4.php'){ ?> checked="checked" <?php } ?> /><label for="home_page_file_4">define_main_page_4.php</label> &nbsp; &nbsp;		
      			<input type="submit" value="update home page">
      		</form>
      	</td>
      </tr>
      <tr><td><a onclick="homepage_wysiwyg();">Open Homepage Wysiwyg Editor</a></td></tr> 
<?php
/* added wysiwyg editor --elibird */
// show editor
...
window.updateSource = function(src){
 //alert(src);
 //document.getElementById('file_contents___Frame').contentDocument.getElementById('xEditingArea').childNodes[0].value=src;
 $('#cke_1_contents').children()[0].value=src;
}

homepage_wysiwyg = function(){
    wysiwindow = window.open('../wysiwyg.html');
}
...
window.sendSource = function(){
    ...
    //wysiwindow.insertSource(document.getElementById('file_contents___Frame').contentDocument.getElementById('xEditingArea').childNodes[0].value);
        
     //which id to pass to wysiwyg.html ??
     var define_it = parse_url()['define_it'];
     var id = '';
     if(define_it == "10"){
         id = "lower_images";
     } else if(define_it == "8"){
         id="indexDefaultMainContent";
     }
    
      wysiwindow.insertSource($('#cke_1_contents').children()[0].value, id);
}

setTimeout(function(){
    if(document.getElementById('file_contents___Frame')){
        //alert('loaded');
        clearTimeout();    
    }
