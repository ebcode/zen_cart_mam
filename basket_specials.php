 ...
 */
  ...
  if(isset($_GET['action'])){
    switch($_GET['action']){
        ...
        break;
        case 'instructions':
            require($_SERVER['DOCUMENT_ROOT'].'/includes/languages/english/html_includes/bookshelf/define_page_4.php');
            die();
            break;
    }
  }
...
//detect $_SESSION['byob'] and offer to empty basket or redirect to checkout
    ...
    $q = "select p.products_image, p.products_image_2, p.products_price, p.sold, p.products_quantity, pd.* from products p left join products_description pd on p.products_id = pd.products_id where p.master_categories_id = '56'";
...
$a = "<a href=\"" . zen_lightbox($tmp_image_file, addslashes($products_name), LARGE_IMAGE_WIDTH, LARGE_IMAGE_HEIGHT) . "\" rel=\"" . $rel . "[]\" title=\"" . ($products_name) . "\">";
        ...
        $y = "<td><form enctype=\"multipart/form-data\" method=\"post\" action=\"https://www.anythinginabasket.com/index.php?main_page=product_info&cPath=36&products_id={$row['products_id']}&number_of_uploads=0&action=add_product\" name=\"cart_quantity\"><input type=\"hidden\" value=\"1\" name=\"cart_quantity\"><input type=\"hidden\" value=\"{$row['products_id']}\" name=\"products_id\"><div class=\"displayitem_container\" style=\"display:inline-block;\" >$a<div  class=\"displayitem\" id=\"item_{$row['products_id']}\" style=\"background: url('./images/{$row['products_image']}') no-repeat scroll center;\"><div class=\"price\">".'$'."{$price}</div><div class=\"hide_me\" id=\"item_{$row['products_id']}_description\"><br>{$row['products_description']}</div>";
        
        if($row['sold'] || $row['products_quantity'] == 0){
         $y .=       "<img src=\"./images/SOLD-1.png\" style=\"\">";
        }     
        
        $y .= "</div></div></a><div class=\"button\"><input type=\"submit\" style=\"display:none; cursor-events:none;\" value=\"add to cart\"></div><div class=\"displayitem_title\">{$row['products_name']}</div></form></td>";
   ...
   require($template->get_template_dir('tpl_byob.php',
        DIR_WS_TEMPLATE, 
        $current_page_base,'templates'). '/tpl_basket_specials.php');
